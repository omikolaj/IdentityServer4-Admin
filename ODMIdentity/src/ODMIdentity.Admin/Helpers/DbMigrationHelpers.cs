...
using ODMIdentity.Admin.Configuration.Identity;
using ODMIdentity.Admin.Configuration.IdentityServer;
using ODMIdentity.Admin.Configuration.Interfaces;
using Skoruba.IdentityServer4.Admin.EntityFramework.Interfaces;

namespace ODMIdentity.Admin.Helpers
{
    public static class DbMigrationHelpers
    {
        ...
        where TRole : IdentityRole, new()
        {
            using (var scope = serviceProvider.GetRequiredService<IServiceScopeFactory>().CreateScope())
            {
                ...
                await EnsureSeedIdentityServerData(context, rootConfiguration.AdminConfiguration, configuration);
        ...
        /// </summary>
        private static async Task EnsureSeedIdentityServerData<TIdentityServerDbContext>(TIdentityServerDbContext context, IAdminConfiguration adminConfiguration, IConfiguration configuration)
            where TIdentityServerDbContext : DbContext, IAdminConfigurationDbContext
        {
            if (!context.Clients.Any())
            {
                foreach (var client in Clients.GetAdminClient(adminConfiguration, configuration).ToList())
                {
